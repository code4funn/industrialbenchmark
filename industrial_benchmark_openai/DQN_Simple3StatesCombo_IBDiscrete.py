...
MINIBATCH_SIZE = 64 * 2 # How many steps (samples) to use for training
UPDATE_TARGET_EVERY = 5  # Terminal states (end of episodes)
MODEL_NAME = 'IB_disc_L3S_D_2_10_'
...
env = OpenAI_IB(setpoint=10, reward_type='classic', action_type='discrete', stationary_p=True)
nb_actions = env.action_space.n
env.seed(123)

# Model shape modification
K_STATES = 3
modified_observation_space_shape = (env.observation_space.shape[0] * K_STATES,)
...
class DQNAgent:
    def __init__(self):
        ...
        self.tensorboard = ModifiedTensorBoard(log_dir="logs-ntest/{}-{}".format(MODEL_NAME, int(time.time())))  #
        # @line 22
    ...
    def create_model(self):
        ...
        model.add(Flatten(input_shape=(1,) + modified_observation_space_shape))
        model.add(Dense(124 * K_STATES))
        model.add(Activation('relu'))
        model.add(Dense(56 * K_STATES))
        model.add(Activation('relu'))
        model.add(Dense(56))
        model.add(Activation('relu'))
    ...
    def train(self, terminal_state, step):
        ...
        current_qs_list = self.model.predict(current_states.reshape(-1, 1, 7 * K_STATES))

        # Get future states from minibatch, then query NN model for Q values
        # When using target network, query it, otherwise main network should be queried
        new_current_states = np.array([transition[3] for transition in minibatch])
        future_qs_list = self.target_model.predict(new_current_states.reshape(-1, 1, 7 * K_STATES))
        ...
        self.model.fit(np.array(X).reshape(MINIBATCH_SIZE, 1, 7 * K_STATES), np.array(y), batch_size=MINIBATCH_SIZE,
                       verbose=0,
                       shuffle=False,
                       callbacks=[
...
for episode in tqdm(range(1, EPISODES + 1), ascii=True, unit='episodes'):
    ...
    # Tile K copies
    current_state = np.tile(current_state, K_STATES)
    ...
    while not done:
        ...
        # Modify the new state by combining the previous two states
        new_state = np.concatenate((current_state[7:], new_state))
