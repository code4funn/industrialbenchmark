...
# import Prioritized Replay Buffer
...
MODEL_NAME = 'IB_disc_PRB_lr.001_b_100_'
...
env = OpenAI_IB(setpoint=100, reward_type='classic', action_type='discrete', reset_after_timestep=1000,
                stationary_p=True)
...
class DQNAgent:
    def __init__(self):
        ...
        self.tensorboard = ModifiedTensorBoard(log_dir="logs-prb/{}-{}".format(MODEL_NAME, int(time.time())))  #
        # @line 22
    ...
    def update_replay_memory(self, transition):
        current_state, action, reward, new_current_state, done = transition

        current_qs = self.model.predict(current_state.reshape(-1, 1, 7))[0]
        future_qs = self.target_model.predict(new_current_state.reshape(-1, 1, 7))[0]
        # print(current_qs)
        # print(future_qs)
        ...
        error = abs(current_qs[action] - new_q)
    ...
    def train(self, terminal_state, step):
        ...
        if self.replay_memory.tree.n_entries < MIN_REPLAY_MEMORY_SIZE:
            return

        # Get a minibatch of random samples from memory replay table
        minibatch, idxs, is_weights = self.replay_memory.sample(MINIBATCH_SIZE)
        ...
        for index, (current_state, action, reward, new_current_state, done) in enumerate(minibatch):
            ...
            # calculate error
            err = abs(current_qs[action] - new_q)
            ...
            # update priority
            idx = idxs[index]
            self.replay_memory.update(idx, err)
